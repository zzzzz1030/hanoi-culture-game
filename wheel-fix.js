...
(function(){
  ...
  if(plusTen){
    plusTen.label = '🎁 +10\nĐỘI VỪA ĐÚNG';
    plusTen.desc = 'Cộng 10 điểm cho tổ có thành viên vừa trả lời đúng câu hỏi';
  }
  ...
  function renderScores(){
    ...
    if(!board) return;
    const maxScore = Math.max(10,...[1,2,3,4].map(t=>Number(scores[t])||0));
  ...
  function applyResult(item){
    ...
    if(item.type === 'add'){
      ...
      text = actual > 0 ? `🔄 Tổ ${winner} lấy ${actual} điểm từ Tổ ${other}!` : `🔄 Tổ ${winner} được chọn lấy điểm từ Tổ ${other}, nhưng Tổ ${other} chưa có điểm.`;
      ...
      text = transfer > 0 ? `🔄 TRAO ĐỔI: Tổ ${a} → Tổ ${b}, chuyển ${transfer} điểm!` : `🔄 TRAO ĐỔI: Tổ ${a} và Tổ ${b} được chọn, nhưng Tổ ${a} chưa có điểm để chuyển.`;
    }
    save();
    updateScore();
  ...
  // Capture trước onclick inline: ghi nhận đúng tổ/người trước khi app.js chuyển câu.
    ...
    if(randomBtn){
      setTimeout(()=>{
        if(Array.isArray(history) && history.length){
          ...
          currentQuestionTeam = Number(picked.team) || null;
