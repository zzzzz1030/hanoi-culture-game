/* HANOI CULTURE GAME - VÒNG QUAY LẶP VÔ HẠN */
(function(){
  'use strict';
  ...
  function renderStatus(){
    ...
    if(!el)return;
    el.className=correctTeam?'correct-team-status has-team':'correct-team-status';
    el.innerHTML=correctTeam
      ? `✅ <b>Tổ ${correctTeam}</b>${correctPerson?` – ${correctPerson}`:''} vừa trả lời đúng.<br><span>🎁 Ô +10 sẽ cộng cho đúng Tổ ${correctTeam}.</span>`
      : '⏳ Chưa có thành viên nào vừa trả lời đúng. Ô 🎁 +10 đang chờ đội trả lời đúng.';
  }
  ...
  function renderScores(){
    ...
    board.innerHTML=[1,2,3,4].map(t=>{
      ...
      return `<div class="wheel-score-card ${correctTeam===t?'is-last':''}"><div class="wheel-score-team">🏆 Tổ ${t}</div>${correctTeam===t?'<small>⭐ Đội vừa trả lời đúng</small>':''}<div class="wheel-score-number">${s} điểm</div><div class="wheel-score-bar"><i style="width:${pct}%"></i></div></div>`;
    }).join('');
  }
  ...
  function renderWheel(){
    ...
    if(legend){legend.innerHTML=wheelItems.map(it=>`<div class="legend-item"><span class="legend-dot" style="background:${it.color}"></span><div><b>${String(it.label).replace(/\n/g,' ')}</b><small>${it.desc}</small></div></div>`).join('');}
  }

  function addPanels(){
    const result=$('wheelResult');
    if(!result)return;
    if(!$('correctTeamStatus')){
      const s=document.createElement('div');s.id='correctTeamStatus';result.parentNode.insertBefore(s,result);
    }
    if(!$('wheelScorePanel')){
      const p=document.createElement('div');p.id='wheelScorePanel';p.className='wheel-score-panel';p.innerHTML='<h2>🏆 Bảng điểm các tổ</h2><div id="wheelScoreboard" class="wheel-scoreboard"></div>';result.parentNode.insertBefore(p,result);
    }
  }
  ...
  // Capture clicks before app.js inline handlers run.
  document.addEventListener('click',function(e){
    const el=e.target&&e.target.closest?e.target.closest('button, .answer'):null;
    if(!el)return;

    if(el.matches('button[onclick*="randomPerson"]')){
      // app.js adds the new person synchronously; read it immediately after the click.
      setTimeout(rememberCurrentPerson,10);
    }
  ...
  if(plusTen){plusTen.label='🎁 +10\nĐỘI VỪA ĐÚNG';plusTen.desc='Cộng 10 điểm cho tổ có thành viên vừa trả lời đúng câu hỏi';}

  function applyResult(item){
    let text='';
    const get=t=>Number(scores[t])||0;
    if(item.type==='add'){
      scores[item.team]=clampScore(get(item.team)+item.points);
      ...
      if(correctTeam){
        scores[correctTeam]=clampScore(get(correctTeam)+10);
        text=`🎉 Tổ ${correctTeam}${correctPerson?` – ${correctPerson}`:''} nhận +10 điểm vì vừa trả lời đúng!`;
      }else text='⚠️ Chưa có tổ vừa trả lời đúng nên ô +10 chưa cộng điểm.';
    }else if(item.type==='subtractRandomTarget'){
      const t=Math.floor(Math.random()*4)+1;
      scores[t]=clampScore(get(t)-10);
      text=`⚠️ Tổ ${t} ngẫu nhiên bị −10 điểm!`;
    }else if(item.type==='takeOther'){
      const a=Math.floor(Math.random()*4)+1;
      const b=[1,2,3,4].filter(t=>t!==a)[Math.floor(Math.random()*3)];
      const n=Math.min(5,get(b));
      scores[b]=get(b)-n;scores[a]=get(a)+n;
      text=n?`🔄 Tổ ${a} lấy ${n} điểm từ Tổ ${b}!`:`🔄 Tổ ${a} được chọn lấy điểm từ Tổ ${b}, nhưng Tổ ${b} chưa có điểm.`;
    }else if(item.type==='exchange'){
      const a=Math.floor(Math.random()*4)+1;
      const b=[1,2,3,4].filter(t=>t!==a)[Math.floor(Math.random()*3)];
      const n=Math.min(5,get(a));
      scores[a]=get(a)-n;scores[b]=get(b)+n;
      text=n?`🔄 TRAO ĐỔI: Tổ ${a} → Tổ ${b}, chuyển ${n} điểm!`:`🔄 TRAO ĐỔI: Tổ ${a} và Tổ ${b} được chọn, nhưng Tổ ${a} chưa có điểm để chuyển.`;
    }
    save();
    updateScore();
    renderScores();
  ...
  function burst(wheel){
    const old=wheel.querySelector('.wheel-result-burst');if(old)old.remove();
    const holder=document.createElement('div');holder.className='wheel-result-burst';
    for(let i=0;i<30;i++){const r=document.createElement('i');r.style.setProperty('--angle',(i*12)+'deg');r.style.setProperty('--distance',(115+Math.random()*75)+'px');holder.appendChild(r);}
    wheel.appendChild(holder);setTimeout(()=>holder.remove(),1200);
  }

  function showCenter(item,message){
    const wheel=$('prizeWheel');if(!wheel)return;
    const old=wheel.querySelector('.wheel-center-result');if(old)old.remove();
    const c=document.createElement('div');c.className='wheel-center-result';
    c.innerHTML=`<div class="wheel-result-title">🎯 KẾT QUẢ</div><div class="wheel-result-label">${String(item.label||'').replace(/\n/g,'<br>')}</div><div class="wheel-result-effect">${message||''}</div>`;
    wheel.appendChild(c);burst(wheel);
  }
  ...
  window.spinWheel=function(){
    ...
    busy=true;button.disabled=true;button.textContent='⏳ ĐANG QUAY...';
    wheel.classList.add('spinning');if(shell)shell.classList.add('spin-flash');
    const old=wheel.querySelector('.wheel-center-result');if(old)old.remove();
    ...
    // Use a guarded timer so every spin always unlocks the button.
    setTimeout(()=>{
      try{
        const item=wheelItems[index];
        const message=applyResult(item);
        result.textContent=message;
        result.classList.remove('result-win');void result.offsetWidth;result.classList.add('result-win');
        showCenter(item,message);
        launchConfetti();
      }catch(err){
        console.error(err);
        result.textContent='⚠️ Không thể xử lý kết quả, nhưng vòng quay đã được mở lại.';
      }finally{
  ...
  addPanels();renderWheel();renderStatus();renderScores();
})();
